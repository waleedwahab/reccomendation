...
function SellEdit() {
    ...
    const [title, setTitle] = useState(locations.state.data.title)
    const [description, setDescription] = useState(locations.state.data.description)
    const [email, setEmail] = useState(locations.state.data.email)
    const [number, setNumber] = useState(locations.state.data.number)
    ...
    return (
        <div className={styles.Main}>
            ...
            <form className={styles.Main2} onSubmit={handleSubmit}>
                ...
                <div className={styles.footer}>  <Button variant='contained' type='submit' sx={{ margin: '15px' }}>Update</Button></div>
            </form>
        </div>
    )
}
